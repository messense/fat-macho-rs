...
impl<'a> FatWriter<'a> {
    ...
    fn write_inner<W: Write>(&self, writer: &mut W, copy: FileCopy<'_, W>) -> Result<(), Error> {
        ...
        // header, then padding + data per arch; a file-backed slice flushes
        // what is pending and is copied on its own
        ...
        for (arch, &arch_offset) in self.arches.iter().zip(&layout.offsets) {
            ...
            match &arch.data {
                ...
                data => bufs.push(IoSlice::new(data.bytes().expect("in-memory slice"))),
            }
